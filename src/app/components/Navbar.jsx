...
export default function Navbar() {
  ...
  return (
    <header
      className={`fixed w-full top-0 z-50 transition-all duration-300 bg-white/80 backdrop-blur border-b border-gray-100 ${scrolled ? 'shadow-md' : ''}`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-3 md:py-4">
          <Link href="/" className="flex items-center space-x-2">
            <span className="text-xl sm:text-2xl md:text-3xl font-bold bg-gradient-to-r from-indigo-600 to-blue-600 bg-clip-text text-transparent">
              ReeOrg
            </span>
          </Link>
          ...
          <nav className="hidden md:flex items-center space-x-4 lg:space-x-8">
            ...
            >
              <button
                id="features-trigger"
                className="text-sm md:text-base font-medium text-gray-700 hover:text-indigo-600 transition-colors flex items-center gap-1"
                aria-haspopup="true"
                  ...
                  <ul className="py-2">
                    {PRODUCT_OPTIONS.map((opt) => (
                      <li key={opt.label}>
                        <Link
                          href={opt.href}
                          className="block px-4 py-2 text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition-colors text-xs md:text-sm"
            ...
            <Link
              href="/signup"
              className="bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-4 md:px-6 py-2 rounded-full text-xs md:text-sm font-semibold hover:shadow-lg hover:opacity-95 transform hover:-translate-y-px transition-all"
            >
              Get Started
            </Link>
          </nav>
      ...
      <div className={`md:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-lg transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-screen border-t border-gray-200' : 'max-h-0'}`}>
          <div className="w-full max-w-screen-2xl mx-auto px-2 py-6 space-y-4">
             <Link
              href="#"
              className="block text-gray-700 hover:text-indigo-600 transition-colors font-medium text-base"
            ...
            <div>
              <button
                className="w-full text-left text-gray-700 hover:text-indigo-600 transition-colors flex items-center justify-between font-medium text-base"
                ...
                <ul className="space-y-1 pl-4 border-l-2 border-indigo-100">
                  {PRODUCT_OPTIONS.map((opt) => (
                    <li key={opt.label}>
                      <Link
                        href={opt.href}
                        className="block px-2 py-2 text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 transition-colors rounded text-sm"
                        onClick={() => setIsOpen(false)}
                      >
                        {opt.label}
                      </Link>
            ...
            <Link
              href="/signup"
              className="block bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-6 py-3 rounded-full text-center font-semibold mt-4 text-base"
              onClick={() => setIsOpen(false)}
            >
              Get Started
            </Link>
