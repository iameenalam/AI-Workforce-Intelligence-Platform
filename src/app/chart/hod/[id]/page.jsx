...
export default function HodPage() {
  ...
  const handleCvUpload = async () => {
    ...
    try {
      const token = Cookies.get("token");
      const res = await fetch(`/api/departments/cv?id=${id}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      ...
      // Silently re-fetch data to update "Reports To" without causing a glitch
      const dataRes = await fetch(`/api/departments/hod/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const fullData = await dataRes.json();
      if (dataRes.ok) {
        setDepartment(fullData);
      }
  ...
  if (!department) return <div className="flex items-center justify-center h-screen bg-slate-50 text-gray-700">No Department found.</div>;

  const education = department.hodEducation || [];
  const experience = department.hodExperience || [];
  const skills = department.hodSkills || [];
  const tools = department.hodTools || [];
  const certifications = department.hodCertifications || [];
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal}>
        <div className="p-4 sm:p-6">
            <h2 className="text-xl font-bold text-gray-800 mb-2">
                {isProfileComplete ? "Update HOD Profile" : "Complete HOD Profile"}
            </h2>
            <p className="text-gray-500 mb-6">
                {isProfileComplete ? "Upload a new CV to update the HOD's details." : "Upload the HOD's CV (PDF) to auto-populate their profile."}
            </p>
          ...
          <div className="bg-white p-6 md:p-8 rounded-2xl shadow-lg mb-8">
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <div className="flex-shrink-0">
                {department.hodPic ? (
                  <img src={department.hodPic} alt={department.hodName} className="w-24 h-24 rounded-full object-cover ring-4 ring-white shadow-md" />
                ) : (
                  <div className="w-24 h-24 bg-indigo-500 text-white rounded-full flex items-center justify-center text-4xl font-bold ring-4 ring-white shadow-md">
                    {department.hodName ? department.hodName[0] : "?"}
                  </div>
                )}
              </div>
              <div className="flex-grow text-center sm:text-left min-w-0">
                <h1 className="truncate text-3xl font-bold text-gray-900">{department.hodName}</h1>
                <p className="text-lg font-medium text-indigo-600">{department.role}</p>
                 <div className="mt-4 flex flex-col items-center sm:items-start gap-2 text-gray-600">
                    <div className="flex flex-wrap justify-center sm:justify-start items-center gap-x-6 gap-y-2">
                        <span className="flex items-center gap-2 truncate"><Mail className="w-5 h-5 text-gray-400" />{department.hodEmail}</span>
                        <span className="flex items-center gap-2 truncate"><Building2 className="w-5 h-5 text-gray-400" />{department.departmentName}</span>
                    </div>
                    <span className="font-medium flex items-center gap-2 truncate"><User className="w-5 h-5 text-gray-400" />Reports To: <span className="font-normal">{department.organization?.ceoName || '-'}</span></span>
                </div>
              </div>
